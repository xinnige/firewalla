...
class APFeaturesPlugin extends Sensor {
  ...
  async _aclIptables(op) {
    ...
    const netRule = new Rule("filter").chn("FW_FIREWALL_NET_ISOLATION").comment("network acl off").jmp("RETURN");
    ...
    const groupRule = new Rule("filter").chn("FW_FIREWALL_DEV_G_ISOLATION").comment("group acl off").jmp("RETURN");
    ...
    const deviceRule = new Rule("filter").chn("FW_FIREWALL_DEV_ISOLATION").comment("device acl off").jmp("RETURN");
  ...
  async _aclAps(op) {
    ...
    const changed = this._setApAcl(config, op);
    if (changed === true) {
      log.info("acl changed, reapply networkConfig", op);
