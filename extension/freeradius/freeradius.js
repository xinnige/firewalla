...
class FreeRadius {
  ...
  async generateDockerCompose(options = {}) {
    ...
    yamlContent.services.freeradius.image = `public.ecr.aws/a0j1s2e9/freeradius:${f.isDevelopmentVersion() ? "dev" : "latest"}`;
  ...
  async upgradeImage(options = {}) {
    try {
      log.info("Checking for new Docker image...");
      if (!options.allow_update) {
        log.debug("Image update is not allowed, skip update");
        return false;
      }

      // get current image digest
      const currentImage = await exec(`sudo docker-compose -f ${dockerDir}/docker-compose.yml images -q freeradius`).then(r => r.stdout.trim()).catch(() => null);
      if (!currentImage) {
        log.info("No freeradius image found");
        return false;
      }
      ...
      // get new iamge digest
      const newImage = await exec(`sudo docker-compose -f ${dockerDir}/docker-compose.yml images -q freeradius`).then(r => r.stdout.trim()).catch(() => null);
      if (!newImage) {
  ...
  async _checkImage(options) {
    const tag = await this._getImageTag(options);
  ...
  async reconfigServer(target, options = {}) {
    ...
    if (!imageUpdated && target != "0.0.0.0" && this.running === true) {
